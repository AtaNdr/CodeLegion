...
- **Live status page.** `/status` on the Web App — Flow 1 setup wizard at the top, Flow 2 fleet dashboard once setup is green. Auto-refreshes. Dark-mode aware.
...
├── LICENSE               MIT.
│
├── index.js              Express entrypoint. Mounts Flow 1 + Flow 2 routes.
...
- **Persistent disk only.** Cost log, activity timelines, raw VM logs, and setup state live in `/home/data/` on the Web App's persistent disk. Single-instance only by design.
...
- **Two flows in the UI.** Flow 1 (setup wizard) appears until all checks are green. Flow 2 (live fleet + cost dashboard) takes over after.
...
- **Cloud-init carries `REPORT_TOKEN`.** Anyone with VM-read on the RG can see it. Token scope is narrow (`/agent/log`, `/agent/status`, `/agent/sync`, `/agent/secrets`) but worth knowing.
